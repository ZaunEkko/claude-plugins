...
`commit-commands@zaunekko`는 Anthropic의 공식 동일 이름 플러그인에서 파생된 제3자 호환 배포입니다. 설치 이름, 명령 네임스페이스와 Git 워크플로를 유지하면서 commit attribution의 `Model:`을 현재 Claude Code 세션 모델과 선택적 effort로 업데이트하고, Claude Code Bash 및 알려진 Playwright unsafe 로컬 프로세스 경로가 attribution wrapper를 우회하지 못하게 합니다.
...
결정적 `PreToolUse` guard는 Bash의 직접 `git commit`, `git.exe commit`, 그리고 `git -C <경로> commit` 같은 일반적인 Git 전역 옵션 형식을 거부합니다. 알려진 Playwright `browser_run_code_unsafe` 도구 이름에 대해서는 정적으로 관찰 가능한 로컬 프로세스 호출을 검사하고, 직접 commit, 명시적인 no-commit 모드가 없는 commit 생성 Git porcelain, attribution wrapper 실행을 차단합니다. 대신 `/commit-commands:commit`, `/commit-commands:commit-push-pr` 또는 Claude Code Bash를 통한 wrapper 호출을 사용하세요.

일반 브라우저 자동화, 읽기 전용 Git 명령, 명시적인 `--no-commit`/`-n`/`--ff-only` 준비 경로는 계속 사용할 수 있습니다. 이 guard는 Git hooks를 설치하지 않고 터미널, IDE, Git GUI 또는 CI에 영향을 주지 않으며, 완전한 shell·JavaScript·임의의 제3자 MCP sandbox가 아닌 워크플로 guard rail입니다.
...
모델은 현재 transcript의 최신 유효 assistant `message.model`, SessionStart model, 현재 프로세스의 `ANTHROPIC_MODEL`, 설정된 기본 `model` 순서로 확인하고 마지막에 생략합니다. 일반 Bash 호출은 내보낸 pointer 또는 `CLAUDE_CODE_SESSION_ID`로 현재 state를 찾습니다. 의도적으로 세션 환경에서 분리된 호출자는 Git 인수 앞에 `--claude-state-file <정확한 경로>`를 전달할 수 있습니다. Wrapper는 해당 비공개 state 파일만 사용하고, 경로가 유효하지 않으면 Git 실행 전에 실패하며, 최신 state를 선택해 동시 세션을 추측하지 않습니다. Effort는 `CLAUDE_EFFORT`, 설정의 `effort`/`effortLevel` 순서입니다. 값은 한 줄 데이터로 검증되며 shell code로 실행되지 않습니다.
