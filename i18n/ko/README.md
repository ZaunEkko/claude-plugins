...
```text
이 Claude Code plugin marketplace의 모든 플러그인을 설치해 주세요:
https://github.com/ZaunEkko/claude-plugins

user, project, local 중 사용할 scope를 확인하고 선택한 scope만 변경하세요. zaunekko marketplace를 추가·업데이트하고 현재 목록을 읽은 뒤 모든 플러그인을 설치하고 활성화하세요. 같은 scope에서 공식 commit-commands가 활성화되어 있다면 네임스페이스 충돌을 설명하고 제거하지 말고 비활성화하세요. ekko-image-gen의 `~/.claude/ekko-image-gen.local.json`이 없으면 `baseUrl`과 자리표시자 `apiKey`만 포함한 템플릿을 만들고 실제 키는 대화에 붙이지 말고 로컬 파일을 직접 편집하도록 안내하세요. 완료 후 /reload-plugins를 실행하고 실제 명령, 결과, 아직 입력해야 할 설정을 보고하세요. 권한, 인증 또는 보안 확인이 필요하면 중단하고 제가 결정하게 해 주세요.
```
...
`~/.claude/ekko-image-gen.local.json`(Windows에서는 `%USERPROFILE%\.claude\ekko-image-gen.local.json`)을 만드세요:
...
endpoint는 localhost 또는 타사 HTTPS 서비스일 수 있습니다. 실제 키를 Git이나 Agent 대화에 붙이지 마세요. 기본 모델과 다중 이미지 짧은 응답은 플러그인이 처리합니다.
