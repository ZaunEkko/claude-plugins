...
### Claude Code のためのパーソナルプラグインマーケットプレイス
...
| `ekko-image-gen` | ローカル利用・オリジナル | 1 つのコマンドでローカル文生図、貼り付けた参照画像の編集、プロジェクト文脈に沿った保存、制限付きリーフ worker、画像レビュー、クリック可能なローカル出力を提供します。 | [利用ガイド](../../docs/ekko-image-gen/README.md) · [実装](../../plugins/ekko-image-gen/README.md) |
...
```text
この Claude Code plugin marketplace をインストールしてください：
https://github.com/ZaunEkko/claude-plugins

対象プラグイン：commit-commands@zaunekko
user、project、local のどの scope を使うか確認し、選択した scope だけを変更してください。同じ scope で公式 commit-commands が有効なら名前空間競合を説明し、アンインストールせず無効化してください。完了後に /reload-plugins を実行し、実際のコマンドと結果を報告してください。権限、認証、セキュリティ確認が必要な場合は停止して私に判断を求めてください。
```
