...
`ekko-image-gen` 是一个面向 Claude Code 的图片生成插件。它通过一个统一命令调用用户配置的 OpenAI-compatible Images API；服务既可以运行在 localhost，也可以是可访问的第三方 HTTPS endpoint。插件支持文生图、粘贴图片后的图生图、多参考图、项目上下文感知落盘、并发 worker、主代理视觉验收，以及默认返回可 Ctrl/Cmd+点击的临时本地 HTTP 预览链接。
...
- 预览 helper 不自动启动浏览器、图片查看器或编辑器；链接带随机 token，只绑定 loopback，并在 15 分钟后自动失效
...
普通用户只需填写 `baseUrl` 和 `apiKey`。`baseUrl` 可以是 localhost，也可以是第三方 HTTPS endpoint；明文 HTTP 只允许 `localhost` 或 loopback 地址，避免把 API Key、提示词和参考图片发送到非本地明文连接。配置每次调用时重新读取，不需要重启 Claude Code；非空环境变量可以覆盖 JSON，空字符串或纯空白值会被忽略。模型、尺寸、并发和 provider 能力都有可用默认值，完整高级字段见 `skills/generate/references/request-schema.md`。
...
```text
/ekko-image-gen:generate 生成一张像素风红色治疗药水图标，透明背景
```

主代理完成 `Read` 检查、验收和必要重试后，会返回绝对文件路径和一个普通 `http://127.0.0.1` 链接。用户可在 Claude Code 终端中 Ctrl/Cmd+点击该链接，由浏览器显示图片；插件不会主动弹出浏览器、图片查看器或编辑器。

链接默认 15 分钟后失效，文件本身不会删除。过期后说“重新给我刚才图片的预览链接”即可复用已验收路径，不会重新生成。一次最多为 4 张验收图创建链接。
...
- [打开图片](http://127.0.0.1:43210/random-token/1/health-potion.png)（15 分钟内有效）
- 服务 URL：http://localhost:3050/images/...png
```

本地 `file://` 链接仍作为兼容信息保留，但不是主要动作。主代理在最终验收后调用 `scripts/preview-image.mjs`，为最多 4 张图片生成带随机 token 的 loopback HTTP URL。普通 HTTP 链接更容易被 Claude Code 和终端识别为可 Ctrl/Cmd+点击的目标。

预览服务只绑定 `127.0.0.1`，只提供请求中的具体图片，不开放目录，并发送 `no-store`、`nosniff`、CSP 与 no-referrer 响应头。15 分钟后进程自动退出。它不会执行浏览器或系统 opener，因此不会主动弹出任何应用。

状态 `ready` 只表示临时链接已经可用。SSH、容器或远程开发中，浏览器所在主机可能无法访问 Claude Code 主机的 loopback 地址；此时仍使用绝对路径。预览链接失败不会把已经成功的图片生成改成 `partial` 或 `error`。
...
- 生成文件写入主代理选择的项目目录。
- 用户发起生成或编辑后，主代理默认在最终验收后启动一个 15 分钟的 loopback-only 预览服务并返回链接；不会主动打开任何 GUI。worker 与后台执行只返回路径。
...
node --check plugins/ekko-image-gen/scripts/preview-image.mjs
node --test plugins/ekko-image-gen/tests/*.mjs
...
测试使用隔离的本地 HTTP server 和 mock detached child，不接触真实 API Key、真实图片服务、用户配置或远程仓库，也不会启动浏览器、图片查看器或编辑器。
